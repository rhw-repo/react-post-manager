...
const debug = require("debug")("materialController:error")
...
const createMaterial = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      debug("[ERROR] Failed to create material:", error.message);
...
const updateMaterial = [
  ...
  async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      debug("[ERROR] Invalid material ID:", id);
      return res.status(404).json({ error: "Document not found" });
    }
    ...
    if (emptyFields.length > 0) {
      debug("[ERROR] Empty fields in update request:", emptyFields);
      return res.status(400).json({
        error: "Please complete all boxes.",
        emptyFields,
      });
    ...
    if (Array.isArray(tags)) {
      ...
      debug("[ERROR] Tags field is not an array.");
      return res
        .status(400)
    ...
    try {
      ...
      debug("[ERROR] Failed to update material:", error.message);
      res.status(400).json({ error: error.message });
    }
  },
];
